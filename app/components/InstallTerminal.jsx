...
export default function InstallTerminal({ variant = 'default' }) {
  ...
  return (
    <div className="land-terminal" style={variant === 'compact' ? { maxWidth: '24rem', margin: 0 } : undefined}>
      <div className="land-terminal-bar">
        <span />
        <span />
        <span />
        <div className="land-terminal-tabs">
          <button className={tab === 'npm' ? 'is-active' : ''} onClick={() => setTab('npm')}>npm</button>
          <button className={tab === 'cdn' ? 'is-active' : ''} onClick={() => setTab('cdn')}>CDN</button>
        </div>
      </div>
      <pre>
